...
export function activate(context: vscode.ExtensionContext) {
    ...
    console.log('Congratulations, your extension "mstranslator" is now active!');

    // The command has been defined in the package.json file
    // Now provide the implementation of the command with  registerCommand
    // The commandId parameter must match the command field in package.json
    let disposable = vscode.commands.registerCommand('mstranslator.translate', async () => {
...
async function translate(text: string): Promise<string> {
    const options = {
        method: 'POST',
        uri: 'https://translator.microsoft.com/neural/api/translator/translate',
        headers: {
            Pragma: 'no-cache',
    ...
    try {
        ...
        return Promise.resolve(response['resultSMT']);
    }
    catch (error) {
        console.log(error)
    }
